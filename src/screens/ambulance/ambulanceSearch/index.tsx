...
export function AmbulanceSearch() {
    ...
    const fetchCoordinates = async (address, isPickup) => {
        const url = `https://maps.googleapis.com/maps/api/geocode/json?address=${encodeURIComponent(address)}&key=${taxidoSettingData?.taxido_values?.location?.google_map_api_key}`;
        try {
            const response = await fetch(url);
            const json = await response.json();
    ...
    const fetchAddressSuggestions = useCallback(async (text) => {
        ...
        const url = `https://maps.googleapis.com/maps/api/place/autocomplete/json?input=${text}&key=${taxidoSettingData?.taxido_values?.location?.google_map_api_key}&types=geocode`;
